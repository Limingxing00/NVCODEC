...
void LeftSix(cv::Mat& temp) {
	//cv::Mat temp2 = temp.clone();
	//cv::MatIterator_<ushort> grayit, grayend;
	//for (grayit = temp.begin<ushort>(), grayend = temp.end<ushort>(); grayit != grayend; grayit++){
	//	*grayit = *grayit << 5;                                                                   // 这就是刚才那个像素的地址，由于是灰度，所以是一个uchar
	//}
	//temp = temp & 0xfff0;
	temp = temp * 32;
	//cv::Mat_<UINT16> ctemp = temp2;
	//ctemp = ctemp << 5;
}

void transform(cv::Mat& temp) {
	temp.convertTo(temp, CV_32FC1);
	cv::Mat pow_temp;
	cv::pow(4480 * temp, 0.355, pow_temp);
	temp = pow_temp - 1;
	temp.convertTo(temp, CV_16UC1);
}
	...
	char *szOutFilePath, char *szOutFilePath2, NvEncoderInitParam *pEncodeCLIOptions, NvEncoderInitParam *pEncodeCLIOptions2)
{
	...
	for (uint i = 1; i < 2; i++) {
		...
		sprintf(OutFilePath, "\\16to10_mapping2_%05d.mkv", i);
