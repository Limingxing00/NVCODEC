...
void split_16bit(cv::Mat &tiff16, cv::Mat &tiff_after, cv::Mat &high8, cv::Mat &low8) {
	// tiff_after:   cv::Mat tiff_after = temp.clone();
	// low8
	const clock_t begin_time1 = clock();
	tiff_after = tiff_after & 0xff;
	//cv::bitwise_and(tiff_after, 0x00ff, tiff_after);
	...
	tiff16 = tiff16 / 256;
	tiff16 -= 0.499999;
	tiff16.convertTo(high8, CV_8UC1);
...
std::vector<std::ofstream*> OutFilePathInfo(char* szOutFilePath, char* szOutFilePath2, int i) {
	...
	sprintf(OutFilePath, "\\H8L8_low%05d.mkv", i);
	sprintf(OutFilePath2, "\\H8L8_high%05d.mkv", i);
	...
	char *szOutFilePath, char *szOutFilePath2, NvEncoderInitParam *pEncodeCLIOptions, NvEncoderInitParam *pEncodeCLIOptions2)
{
	...
	for (uint i = 0; i < 1; i++) {
		...
		for (uint j = 0; j < dimension[2] + 4; j++) {//dimension[2]+4
			...
			if (j < dimension[2]) {
				...
				if (!temp.empty()) {
					...
					//if (j == 530) {
					//	cv::imwrite("C:\\Users\\USER\\Desktop\\opencv\\high_xie2_after2.tiff", high);
					//	cv::imwrite("C:\\Users\\USER\\Desktop\\opencv\\low_xie2_after2.tiff", low);
					//	cv::imwrite("C:\\Users\\USER\\Desktop\\opencv\\test.tiff", tiff_after2);
					//	std::cout << "Succeed stract!" << std::endl;
					//}
		...
		//FILE *pFile;
		//char *file;
		//file = "C:/Users/USER/Desktop/H8L8_high00001.mkv";
		//pFile = fopen(file, "rb");  //获取已打开文件的指针
		//fseek(pFile, 0, SEEK_END);  //先用fseek将文件指针移到文件末尾
		//uint n = ftell(pFile);    //再用ftell获取文件内指针当前的文件位置。
		//						 //这个位置就是文件大小。
		//FILE *pFile2;
		//char *file2;
		//file2 = "C:/Users/USER/Desktop/H8L8_low00001.mkv";
		//pFile2 = fopen(file2, "rb");  //获取已打开文件的指针
		//fseek(pFile2, 0, SEEK_END);  //先用fseek将文件指针移到文件末尾
		//uint n2 = ftell(pFile2);    //再用ftell获取文件内指针当前的文件位置。
		//						 //这个位置就是文件大小。
		//uint n3 = n + n2;
		//float sum = 2530476032;
	
		std::cout << "总共时间是 " << (float(clock() - begin) / CLOCKS_PER_SEC) << std::endl;
		//std::cout << "The compression ratio is "<< sum/n3 << std::endl;
		...
		double mean6 = sum_t6;
		std::cout << "write " << sum_t6 << std::endl;
	}
