...
inline void split_16bit(cv::Mat &tiff16, cv::Mat &tiff_after, cv::Mat &high10, cv::Mat &low8){
	

	// LOW 6
	const clock_t begin_time1 = clock();
	low8 = tiff_after & 0x003f;
	low8.convertTo(low8, CV_8UC1);

	// HIGH 10
	const clock_t begin_time2 = clock();
	high10 = tiff16 & 0xffc0;
	//high8 = tiff16 / 1024;
...
std::vector<std::ofstream*> OutFilePathInfo(char* szOutFilePath, char* szOutFilePath2, int i) {
	...
	sprintf(OutFilePath, "\\H10L6_low%05d.mkv", i);
	sprintf(OutFilePath2, "\\H10L6_high%05d.mkv", i);
	...
	char *szOutFilePath, char *szOutFilePath2, NvEncoderInitParam *pEncodeCLIOptions, NvEncoderInitParam *pEncodeCLIOptions2)
{
	...
	for (uint i = 1; i < 2; i++) {
		...
		// 低8位 lossy
		NvEncoderCuda enc(cuContext, nWidth, nHeight, eFormat_8);
		...
		pEncodeCLIOptions->SetInitParams(&initializeParams, eFormat_8);

		enc.CreateEncoder(&initializeParams);																							// 近乎 走完了NvEncoder.cpp全部的

		//----------------------------------------------------------我的部分-----------------------------------------------------------------------------------------------
		// 高10位 lossless
		NvEncoderCuda enc2(cuContext, nWidth, nHeight, eFormat_10);
		...
		pEncodeCLIOptions2->SetInitParams(&initializeParams2, eFormat_10);
		...
		for (uint j = 0; j < dimension[2] + 4; j++) {//dimension[2]+4
			...
			if (j < dimension[2]) {
				...
				if (!temp.empty())  {
					...
					// grey to nv12
					grey2nv12_10(high, high_nv12);
					grey2nv12_8(low, low_nv12);



					const clock_t begin_time1 = clock();
					std::memcpy(pHostFrame2.get(), high_nv12.datastart, 2048 * 788 * 3);
...
int main(int argc, char **argv) //这个意思就是cmd 输入
{
	...
	try
	{
		...
		encodeCLIOptions2 = NvEncoderInitParam("-codec hevc -preset lossless -profile main10");
