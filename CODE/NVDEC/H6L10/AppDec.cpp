	...
	const Rect &cropRect, const Dim &resizeDim)
{
	...
	cv::Mat buffer = cv::Mat::zeros(788 * 3 / 2, 2048, CV_8UC1);
	cv::Mat buffer2 = cv::Mat::zeros(788 * 3 / 2, 2048, CV_16UC1);
	cv::Mat grey;
	cv::Mat grey2;
	...
	do {
		...
		t3.push_back(float(clock() - begin_time3) / CLOCKS_PER_SEC);
			...
			// high 6
			cv::cvtColor(buffer, grey, 106);
			grey.convertTo(grey, CV_16UC1);
			
			// low 10
			cv::Mat roi(buffer2, cv::Rect(0, 0, 2048, 788));
